...
const double BALL_Y_POS =
    SCREEN_SIZE_Y - 1 / 5.0 * SCREEN_SIZE_Y - BALL_RADIUS - Y_START_OF_BOXES;
...
void create_half_desructive_collision(scene_t *scene, double elasticity,
                                      body_t *keep, body_t *erase) {
  collision_event_aux_t *aux = malloc(sizeof(collision_event_aux_t));
  aux->elasticity = elasticity;
  create_collision(scene, keep, erase,
                   (collision_handler_t)half_destructive_handler, aux, free);
}
...
void create_boxes(scene_t *scene, body_t *ball) {
  ...
  for (int i = 0; i < NUMBER_OF_BOXES_PER_COLUMN; i++) {
    for (int j = 0; j < NUMBER_OF_BOXES_PER_ROW; j++) {
      ...
      rgb_color_t color =
          rgb_from_hsv_init(BOXES_RANGE_OF_COLORS / NUMBER_OF_BOXES_PER_ROW * j,
                            BOXES_COLOR_SATURATION, BOXES_COLOR_HUE);
      body_t *box_body = body_init(box, BOX_MASS, color);
      double x_pos =
          ROW_SPACE_BETWEEN_BOXES * (j + 1) + (1.0 / 2 + j) * box_width;
      double y_pos = SCREEN_SIZE_Y -
                     (((1.0 + COLUMN_SPACE_BETWEEN_BOXES) * i + 0.5 * i) /
                      COLUMN_SPACE_BETWEEN_BOXES) *
                         box_height -
                     Y_START_OF_BOXES;
...
body_t *make_paddle(scene_t *scene) {
  ...
  body_t *paddle_body =
      body_init(paddle, PADDLE_MASS, rgb_init_random_bright());
  vector_t position = {.x = PADDLE_X_POS, .y = PADDLE_Y_POS};
