...
void add_star(size_t num_points) {
  list_t *star_shape =
      sprite_make_star(num_points, STAR_MIN_LENGTH, STAR_MAX_LENGTH);
  rgb_color_t color = rgb_init_random_bright();
  vector_t initial_position = {.x = INITIAL_POSITION_X,
                               .y = INITIAL_POSITION_Y};
  vector_t initial_velocity = {.x = INITIAL_VELOCITY_X,
...
void add_ground() {
  ...
  vector_t initial_position = {.x = SCREEN_SIZE_X / 2.0,
                               .y = GROUND_HEIGHT / 2.0};
