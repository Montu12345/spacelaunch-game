...
enum body_type_t { bullet_t, defender_t, invader_t };
...
void bullet_defender_collision(scene_t *scene, body_t *bullets,
                               body_t *defender) {
  body_t *first_body = scene_get_body(scene, 0);
  bool defender_exists =
      *(enum body_type_t *)body_get_info(first_body) == defender_t;
  if (defender_exists) {
    create_destructive_collision(scene, defender, bullets);
  }
}

void invader_defender_collision(scene_t *scene, body_t *invader,
                                body_t *defender) {
...
void make_defender_bullets(scene_t *scene, body_t *defender) {
  ...
  body_t *bullet_body = body_init_with_info(bullet, BULLET_MASS, BULLET_COLOR,
                                            body_type_init(bullet_t), free);
...
void make_invaders(scene_t *scene, body_t *defender) {
  for (int i = 0; i < INVADERS_PER_ROW; i++) {
    for (int j = 0; j < INVADERS_PER_COLUMN; j++) {
      list_t *invader =
          sprite_make_invader(INVADER_RADIUS, INVADER_BOTTOM_DEPTH);
      body_t *invader_body =
          body_init_with_info(invader, INVADER_MASS, rgb_init_random_bright(),
                              body_type_init(invader_t), free);
      vector_t position = {
          .x = SCREEN_SIZE_X * i / (INVADERS_PER_ROW) + INVADER_RADIUS,
...
void teleport_invaders(scene_t *scene) {
  for (size_t i = 0; i < scene_bodies(scene); i = i + 1) {
    ...
    bool invader_off_right =
        centroid.x + INVADER_RADIUS > SCREEN_SIZE_X && velocity.x > 0;
...
void handle(char key, key_event_type_t type, double held_time,
            space_aux_t *aux) {
...
body_t *make_defender(scene_t *scene) {
  list_t *defender =
      sprite_make_ellipse(DEFENDER_MAX_LENGTH, DEFENDER_MIN_LENGTH);
  body_t *defender_body =
      body_init_with_info(defender, DEFENDER_MASS, rgb_init_random_bright(),
                          body_type_init(defender_t), free);
  vector_t position = {.x = DEFENDER_X_POS, .y = DEFENDER_Y_POS};
