...
size_t num_balls()
{
    return SCREEN_SIZE_X / (BALL_RADIUS * 2);
}
...
void make_anchor(scene_t *scene, int idx)
{
    ...
    vector_t anchor_pos = {.x = BALL_RADIUS * 2 * idx + BALL_RADIUS, .y = SCREEN_SIZE_Y / 2};
...
void make_bodies(scene_t *scene)
{
    for (size_t i = 0; i < num_balls(); i++)
    {
        ...
        vector_t position = {.x = BALL_RADIUS * 2 * i + BALL_RADIUS, .y = SCREEN_SIZE_Y / 2};
...
void move_bodies(scene_t *scene)
{
    ...
    {
        create_spring(scene, HOOKES_CONSTANT, scene_get_body(scene, i), scene_get_body(scene, i + 1));
