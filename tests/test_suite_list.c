...
{
    list_t *l = list_init(1, (free_func_t)vec_free);
    assert(list_size(l) == 0);
    // Add
    ...
    *v = VEC_ZERO;
    ...
    // Remove
    assert((vector_t *)list_remove(l, list_size(l) - 1) == v);
    free(v);
    assert(list_size(l) == 0);
    // Add again
    v = malloc(sizeof(*v));
...
{
    list_t *l = list_init(5, (free_func_t)vec_free);
    assert(list_size(l) == 0);
    // Fill partially
    vector_t *v = malloc(sizeof(*v));
    *v = VEC_ZERO;
...
{
    list_t *l = list_init(LARGE_SIZE, (free_func_t)vec_free);
    // Add to capacity
...
void test_list_large_add_remove()
{
    ...
    for (size_t i = 1; i <= LARGE_SIZE; i++)
    {
        ...
        vector_t *v = list_remove(l, list_size(l) - 1);
    ...
    for (size_t i = 0; i < LARGE_SIZE; i++)
    {
        assert(vec_equal(*(vector_t *)list_get(l, i), (vector_t){i + 1, i + 1}));
    }
    list_free(l);
}
...
void test_out_of_bounds_access()
{
    ...
    access->list = list_init(max_size, (free_func_t)vec_free);
    ...
    for (size_t size = 0; size <= max_size; size++)
    {
        ...
        for (access->index = 0; access->index < size; access->index++)
        {
            ...
            *(vector_t *)list_get(access->list, access->index) = new_vector;
            assert(vec_equal(*(vector_t *)list_get(access->list, access->index), new_vector));
        }
...
{
    const size_t size = 3;
    list_t *l = list_init(size, (free_func_t)vec_free);
...
{
    list_remove((list_t *)l, list_size(l));
}
void test_empty_remove()
{
    ...
    for (size_t i = 0; i < size; i++)
    {
        vector_t *list_v = list_remove(l, list_size(l) - 1);
...
void test_list_get()
{
    ...
    assert(vec_isclose(v, *(vector_t *)list_get(l, 1)));
    list_free(l);
}
